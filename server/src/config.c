...
void read_config(const char *filename, Config *config) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        } else if (strncmp(line, "mysql_user =", 12) == 0) {
            sscanf(line, "mysql_user = %s", config->mysql_user);
        } else if (strncmp(line, "mysql_password =", 16) == 0) {
            sscanf(line, "mysql_password = %s", config->mysql_password);
        } else if (strncmp(line, "mysql_db =", 10) == 0) {
            sscanf(line, "mysql_db = %s", config->mysql_db);
